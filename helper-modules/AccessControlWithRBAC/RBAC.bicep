...
resource storageAccount 'Microsoft.Storage/storageAccounts@2025-01-01' existing = if (RBACSettings.?storageAccountSettings != null) {
  name: RBACSettings.storageAccountSettings!.name
}

resource serviceBusNamespace 'Microsoft.ServiceBus/namespaces@2024-01-01' existing = if (RBACSettings.?serviceBusNamespaceSettings != null) {
  name: RBACSettings.serviceBusNamespaceSettings!.name
}

resource apim 'Microsoft.ApiManagement/service@2024-06-01-preview' existing = if (RBACSettings.?apiManagementSettings != null) {
  name: RBACSettings.apiManagementSettings!.name
}

resource keyVault 'Microsoft.KeyVault/vaults@2025-05-01' existing = if (RBACSettings.?keyVaultSettings != null) {
...
resource sqlServer 'Microsoft.Sql/servers@2024-11-01-preview' existing = if (RBACSettings.?sqlServerSettings != null) {
  name: RBACSettings.sqlServerSettings!.name
}

resource redisCache 'Microsoft.Cache/redis@2024-11-01' existing = if (RBACSettings.?redisCacheSettings != null) {
...
resource webApp 'Microsoft.Web/sites@2025-03-01' existing = if (RBACSettings.?webAppSettings != null) {
  name: RBACSettings.webAppSettings!.name
}

resource containerApp 'Microsoft.App/containerApps@2025-10-02-preview' existing = if (RBACSettings.?containerAppSettings != null) {
