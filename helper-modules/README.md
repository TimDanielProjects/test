...
| Logic App API Connection | `LogicApp_Modules/LogicAppApiConnection.bicep` | ARM-provisioned managed API connection for Logic Apps (e.g. File System) |
...
XML policy files for API Management are co-located in the `policies/` directory:

| File | Description |
|------|-------------|
| `policies/allOperations.policy.xml` | Default all-operations policy |
| `policies/allApiPolicy.xml` | All-API policy with Nodinite logging fragment |
| `policies/genericNodiniteLoggingPolicyFragment.xml` | Reusable Nodinite logging policy fragment |

These are loaded at deploy time via `loadTextContent()` from `LoggingSettings/NodiniteSettings.bicep` and generated API templates.
...
The `logicapp-connections-catalog.json` file defines all available Logic App connections that Platypal can scaffold. Each entry includes:

- **`id`** — unique identifier (e.g. `AzureBlob`, `serviceBus`, `Sftp`)
- **`category`** — `ServiceProvider`, `ManagedApi`, `ApiManagement`, or `Function`
- **`connectionJson`** — the verbatim JSON block written into `connections.json`
- **`appSettings`** — Bicep expressions for environment variable generation
- **`derivedFrom`** — where connection settings come from (`integrationStorage`, `sharedServiceBus`, `manual`, etc.)

To add a new connection type, add an entry to this catalog file. Platypal will automatically show it in the connection selection dialog.
